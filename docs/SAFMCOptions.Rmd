...
Management of multispecies complexes in a resource-limited context was a key concern identified by SAFMC staff. Resource limitations mean fewer and lower frequency of stock assessment reports in the South Atlantic relative to other regions; for the Snapper-Grouper FMP there are 15 assessments but 55 species under management. To improve management of multispecies complexes, investigation and testing of multispecies reference points that can account for species relationships and habitat was recommended. Staff recognized single species MSY as unachieveable for all species in a large complex, so reference points considering tradeoffs between species as well as inherent data limitations are needed.
...
The actual list of management measures would be determined by SAFMC prior to beginning any analyses. Example management measures could include status quo management (single species reference points, assessments, and management measures for some species in the multispecies fishery) and a range of options for multispecies management. One option for multispecies management could combine a limit on removals for the entire multispecies complex based on a multispecies fishing mortality reference point with minimum biomass thresholds at the species level for assessed species in the multispecies fishery, similar to the measures proposed for New England FEP [@fogarty_art_2014]. Additional management measures could be adapted from single species to multispecies use. For example, previous MSEs have evaluated simplified management procedures at the stock level in the South Atlantic region [@peterson_climate-readiness_2025].
 
### Uncertainties 
Actual uncertainties would be determined by SAFMC prior to beginning any analyses. Example uncertainties could include changes in the prevailing environmental conditions, changes in species interactions, availability of stock level productivity information, and availability of timely and correct fishery information.

### Operating Model 
The SAFMC full food web model (South Atlantic Region (SAR) EwE Model) could be modified to serve as an operating model for estimating and evaluating multispecies reference points for different species complexes. This model serves as the basis for the SAR MICE (model of intermediate complexity) focused on the Snapper Grouper complex (SAFMC EwE MICE Model Technichal Report June 2022, and [SSC Report October 2021](https://safmc.net/documents/a08b_ewe_red-snapper-workshop-findings-october-ssc_revised-pdf/)).  

Focus the model for the issue: Aggregate around the appropriate multispecies fishery but keep components of that fishery separate in the operating model and any critical species links to the focal managed complex

Add any necessary detail to the model fishery components to enable simulation of fishing patterns associated with the management measures (e.g. characterize key commercial and recreational "fleets"). 

Consider which environmental drivers, if any, would be needed to address uncertainties and outline how to implement them. For example, if distributions for some stocks are expected to shift outside the region, consider forcing this in the model using declining biomass within the model bounds over time as one scenario. Another example might be an increase in productivity over time for "climate winners" identified in the South Atlantic CVA and a corresponding decrease in productivity for "climate losers." 

Consider the "assessment" method; in the food web model it will be a shortcut method that adds uncertainty or bias to the true operating model dynamics. More uncertainty or bias can be added to simulate data availability. 
Convert the model to Rpath to use the MSE capabilities. Food web static and dynamic parameters are inputs, start at the "current year" endpoint of any fitted SAFMC run using historic data for simplicity. MSE projections can start from that endpoint using fitted parameters as inputs


### Simulations 

Include parameter uncertainty in the Rpath simulations for any given scenario [@whitehouse_assessing_2020; @whitehouse_bottomup_2021]. 
Look at both short term and long term dynamics
