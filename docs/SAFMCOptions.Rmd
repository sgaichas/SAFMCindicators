...
An ocean reanalysis consists of an ocean physics model that has incorporated ("assimilated") observational data collected from many instruments measuring the ocean. The model adjusts to match observational data where it exists, and fills in areas without observational data. While the general patterns in GLORYS compare well with independently collected ocean data, there can be differences at fine scales and in highly dynamic areas. Therefore, GLORYS is more appropriate for some applications than others, and comparisons with in situ measurements are encouraged. Validation of this model-based temperature indicator could be achieved with an expanded Citizen Science project; establishment of a formal Study Fleet such as that in the Northeast US [@jones_learning_2022] will be outlined if the indicator shows promise.  
