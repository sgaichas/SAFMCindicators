...
## Purpose and Context
This report provides a comprehensive review of ecosystem data products and their use across all eight U.S. Regional Fishery Management Councils to support the South Atlantic Fishery Management Council (SAFMC) in enhancing its ecosystem-based fisheries management approaches. The review synthesized literature, examined data products, and conducted semi-structured interviews with Council staff. The work addresses a key priority of the East Coast Coordination Group: identifying ecosystem information that can improve management responsiveness and resilience under changing ocean conditions.
...
### Three General Ecosystem Approaches
...
However, CVA information is underutilized. Only the Mid-Atlantic currently incorporates CVA results systematically into risk assessment and catch advice processes, but New England and the Pacific are developing processes to use this information.
...
- **Habitat-Ecosystem Integration**: Ecosystem approaches and some indicators are identified in comprehensive policy statements 
- **Citizen Science**: Proactive incorporation of participatory data collection 
- **Existing Ecosystem Products**: Recent ESR, fish CVA, and fishing community CVA provide ready-to-use information
...
1. **Leverage Existing Products**: Align indicators from the South Atlantic ESR with objectives in EFH policy documents and CVA results to evaluate whether an integrated risk assessment framework could be developed

2. **Hybrid Approach**: Given mix of data-rich and data-limited stocks across diverse habitats, combining aspects of successful practices from different Councils has more potential than adopting a particular Council's ecosystem approach
...
4. **Update Reporting Frequency**: Work toward more regular ecosystem reporting focused on Council-derived objectives and associated indicators produced with streamlined automation processes developed for the Caribbean ESR
...
6. **Explore Novel Approaches**: Evaluate multispecies management strategies using the existing food web model, considering both commercial yield and recreational fishing opportunity objectives
<!--
---

**Full report contains**: Detailed product comparisons across all Councils, comprehensive indicator tables, and semi-structured interview results.
-->
...
The primary objective of structured interviews was to determine what the Councils think about their ecosystem information products and processes.
...
A challenge associated with the risk table process is that some Council participants that did not engage directly in the pilot process may have questions about the extent to which risk tables will add value for management. While future FEP initiatives might consider additional ways to improve the responsiveness of fisheries management to climate variability/change, including developing climate and ecosystem indicators to inform fishery management decisions at the FMP level, PFMC is currently focusing on implementation of its Inflation Reduction Act projects before selecting a new FEP initiative.
...
The Gulf Council manages under 6 FMPs; 4 [multispecies](https://gulf-council-media.s3.amazonaws.com/uploads/2025/02/Species-Managed-03_2018_shortversion-1.pdf) (Coastal migratory pelagics (3 species), Reef fish (31 species), Shrimp (4 species), Coral (140+ species)), 2 single species (Red drum, Spiny lobster). A Generic Essential Fish Habitat Plan is updated to address EFH across the other FMPs. The Coastal migratory pelagics and Spiny lobster FMPs are joint with the South Atlantic Council, as was the original Coral FMP. 
...
The Council develops [Fishery Performance Reports](https://safmc-shinyapps.shinyapps.io/FPRAll/) from its FMP Advisory Panels in association with updated stock assessments. These reports include information on landings/discard trends, management measure performance, environmental and socioeconomic influences on the fishery, and any other concerns. In 2022, The Council adjusted Fishery Performance Report discussion questions to gather information for use in allocation decisions using [Allocation Decision Trees](https://safmc.net/documents/safmc_allocationdecisiontoolblueprint_202312_final/), although use of this tool may not continue. The adjustments specifically address species distribution shifts, fishing practices related to catch and release, and social or cultural importance. 
...
The International Council for the Exploration of the Seas (ICES) is an intergovernmental organization of 19 member countries that coordinates, advances, and shares marine science in support of sustainable management and conservation. ICES provides [scientific advice](https://ices-taf.shinyapps.io/advicexplorer/) on catch for over 400 fish stocks in [17 ecoregions](https://www.ices.dk/advice/ICES%20ecoregions%20and%20advisory%20areas/Pages/ICES-ecosystems-and-advisory-areas.aspx) of the northeast Atlantic Ocean. This advice draws on the collaborative work of scientists from institutions throughout Europe and North America.

ICES has developed methods for including environmental indicators in catch advice for the Irish Sea [@ices_report_2019; @bentley_refining_2021; @howell_combining_2021]. The method combines standard single species assessments with an ecosystem model integrating stock specific indicators. The stock assessments provide a range of potential allowable catches under Europe's standard fishery management approach, and the ecosystem model provides information on whether the stock is experiencing favorable or unfavorable ecosystem conditions. The ecosystem model's conditions are used to identify where in the acceptable range catch should be set: the lower part of the acceptable range is advised under unfavorable conditions, while the upper part of the range is advised during favorable conditions [@bentley_refining_2021]. The resulting fishing mortality is called $F_{eco}$. This approach makes practical use of available ecosystem information for the stock, while remaining within the typical assessment and management decision process [@howell_combining_2021]. An example for Irish Sea cod uses a temperature indicator to scale assessment based fishing mortaltity to $F_{eco}$ (Fig.\@ref(fig:ICESFeco), [@ices_cod_2025]).

```{r ICESFeco, fig.cap="Reprinted from ICES 2025. Cod in Division 7.a. Derivation of Feco. Left: time-series of inverted sea-surface temperature (SST; three-year lag) rescaled between zero and one, which informs the status of the indicator (Is) in 2024 and 2025 compared with previous years. Right: the status of the indicator determines the placement of the Feco reference point within FMSY ranges.", out.width="50%"}
knitr::include_graphics(here::here("docs/images/codFeco2025.png"))
```

ICES produces both stock level catch advice and information at the ecoregion level on fisheries [@ices_celtic_2025] and ecosystem level drivers and pressures [@ices_celtic_2022]. The $F_{eco}$ approach draws on the full range of ICES science products as well as existing ecosystem models for the Irish Sea [@bentley_retrospective_2020]. Importantly, the Irish Sea food web model was improved using diet and historical catch information from experienced regional fishers [@bentley_combining_2019; @bentley_fishers_2019]. 

#### National Marine Sanctuaries

Literature review found that there are ESR like products used for assessment and strategic planning in the National Marine Sanctuaries, as well as CVAs for several Sanctuaries. These could be summarized in the future if of interest, although the management objectives are much more different from those of Fishery Management Councils. The above examples were considered most relevant for the SAFMC.
...
SAFMC is unique among Councils in explicitly linking EFH and ecosystem efforts, and in proactively bringing citizen science into its process. These advantages can be incorporated into approaches going forward. An initial step may be for the Council to revisit its ecosystem goals (adopted in FEP I):  
...
These are very broad, and explicit objectives could be developed under each goal. Ecosystem management objectives have been developed independently of the broader ecosystem goals and in the context of designating EC species for the Dolphin-Wahoo and Snapper Grouper FMPs. Further coordination across FMPs may be feasible. 
...
Phase 2 of this project will expand on these and other ideas. 
